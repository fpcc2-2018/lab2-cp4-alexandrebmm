...
Alteramos o *script* R `import-events_to_searches.R` que foi pré-disponibilizado para esta atividade. Nele foi incluída uma nova variável para o tempo de duração de sessão (`session_length`), que será utilizada na pergunta 4.
...
* `session_length` - Tempo de duração de uma sessão
* `date` - Variável que indica a data do evento
...
* `date` - Data em que ocorreu o evento, no formato YYYY-mm-dd
...
A "duração da sessão" foi sumarizada na variável `session_length`, a partir da diferença entre o final de uma sessão e o seu início.
...
Observamos, através do gráfico apresentado acima, que o Grupo B tende a ter sessões de busca maiores do que o Grupo A.